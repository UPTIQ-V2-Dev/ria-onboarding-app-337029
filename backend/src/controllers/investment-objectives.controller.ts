...
const getInvestmentObjectivesByClientId = catchAsyncWithAuth(async (req, res) => {
    const investmentObjectives = await investmentObjectivesService.getInvestmentObjectivesByClientId(
        req.params.clientId
    );
    if (!investmentObjectives) {
...
const updateInvestmentObjectivesById = catchAsyncWithAuth(async (req, res) => {
    const investmentObjectives = await investmentObjectivesService.updateInvestmentObjectivesById(
        req.params.id,
        req.body
    );
    res.send(investmentObjectives);
...
const updateInvestmentObjectivesByClientId = catchAsyncWithAuth(async (req, res) => {
    const investmentObjectives = await investmentObjectivesService.updateInvestmentObjectivesByClientId(
        req.params.clientId,
        req.body
    );
    res.send(investmentObjectives);
