 ...
 */
const getOnboardingDataByClientId = async (clientId) => {
 ...
 */
const updateOnboardingStep = async (clientId, stepNumber, stepData) => {
    const existingOnboardingData = await prisma.onboardingData.findUnique({
        where: { clientId }
    });
    if (!existingOnboardingData) {
    ...
    if (updatedOnboardingData.personalInfo && typeof updatedOnboardingData.personalInfo === 'object') {
        const personalInfo = updatedOnboardingData.personalInfo;
        if (personalInfo.socialSecurityNumber) {
            personalInfo.socialSecurityNumber = `***-**-${personalInfo.socialSecurityNumber.slice(-4)}`;
        }
    }
 ...
 */
const getOnboardingSummary = async (clientId) => {
    const onboardingData = await prisma.onboardingData.findUnique({
        where: { clientId }
    });
    if (!onboardingData) {
    ...
    const uploadedDocuments = onboardingData.uploadedDocuments || [];
    ...
    const complianceRecords = onboardingData.complianceRecords || [];
    const complianceStatus = {
        total: complianceRecords.length,
        completed: complianceRecords.filter(record => record.acknowledged).length
    };
    // Calculate total funding
    const fundingMethods = onboardingData.fundingMethods || [];
    ...
    const selectedAccountTypes = onboardingData.selectedAccountTypes || [];
    ...
    let personalInfo = onboardingData.personalInfo;
...
const submitOnboarding = async (data) => {
    ...
    await prisma.onboardingData.update({
        ...
        data: {
            ...
            submittedAt,
            complianceRecords: [
                ...onboardingData.complianceRecords,
                {
